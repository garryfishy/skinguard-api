...
const HF_ROUTER_ENDPOINT = process.env.HF_ROUTER_ENDPOINT || 'https://router.huggingface.co/v1/chat/completions';
...
async function analyzeIngredients(text) {
  ...
  try {
    ...
    if (!response.ok) {
      const message = String(payload?.error || `Hugging Face request failed with status ${response.status}`);
